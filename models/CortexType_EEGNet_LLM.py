...
# pip install datasets braindecode accelerate -U # installation needed 
#############################################################################
#################################### END ####################################
#############################################################################
...
"""
Using the pre-trained model as a frozen feature extractor in a Scikit-learn classification pipeline. 
For this, we first have to get a frozen feature extractor from our classification neural network. 
To do that, we define two function: one that discards the classification layers 
and only keep the embedding part of the model, and one that freezes the model to avoid accumulating gradients unnecessarily.
"""
from collections import OrderedDict
from torch import nn
...
"""
Now we have a frozen pytorch model to act as frozen embedding function, 
we want to integrate it in a Scikit-learn pipeline. For this, 
we need to wrap it in a sklearn.base.TransformerMixin. 
Unfortunately, Skorch does not implement this kind of estimators, so we have to define it. 
"""
from skorch import NeuralNet
from skorch.utils import to_numpy
from sklearn.base import TransformerMixin
...
"""
And finally, we define a function that flattens all the dimensions of its input except 
for the first one (i.e. the batch dimension). 
This way, we will be able to pass those frozen features to Scikit-learn classifiers.
"""
...
model_gpt2_finetuned = GPT2LMHeadModel.from_pretrained(model_path)
tokenizer_gpt2_finetuned = GPT2Tokenizer.from_pretrained(model_path)

def predict_next_char_with_confidence(text, board, tokenizer, model):
    # input_text = text[-sequence_length:]
...
def decide_final_prediction(eeg_char, eeg_prob, bert_char,
                            bert_prob, gpt2_char, gpt2_prob,
                            gpt2_finetuned_char, gpt2_finetuned_prob):
    predictions = {
        eeg_char: eeg_prob,
        bert_char: bert_prob,
        gpt2_char: gpt2_prob,
...
class CortexType(RenaScript):
    def __init__(self, *args, **kwargs):
        ...
        #################################### NEW ####################################
        #############################################################################
        self.model = Pipeline([('embedding', FrozenNeuralNetTransformer(embedding)),
    ...
    def train_epochs(self):
        ...
        #x_train = x_train.reshape(x_train.shape[0], -1) ## check if we need this 
        #x_test = x_test.reshape(x_test.shape[0], -1) ## check if we need this 
    ...
    def predict(self):
        ...
        # GPT-2 finetuned
        gpt2_finetuned_predicted_char, gpt2_finetuned_prob = predict_next_char_with_confidence(self.accumulated_text, Board, tokenizer_gpt2_finetuned, model_gpt2_finetuned)
        ...
        predicted_letter = decide_final_prediction(eeg_predicted_char, y,
                                                   ...
                                                   gpt2_finetuned_predicted_char, gpt2_finetuned_prob)
        ...
        print('GPT-2 Finetuned Predicted:', gpt2_finetuned_predicted_char, 'with probability', round(gpt2_finetuned_prob, 3))
